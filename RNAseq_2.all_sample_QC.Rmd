...
Load count files
```{r}
all_files=as_tibble(list.files(pattern = '.ReadsPerGene_noRM.txt'))%>%rename(sampleName=value)
sampleTable=select(all_files,sampleName)
sampleTable$fileName=sampleTable$sampleName
sampleTable$sampleName=substr(sampleTable$sampleName,12,15)
